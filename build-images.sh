...
reponame="dokuwiki"
dokuwiki_image="docker.io/dokuwiki/dokuwiki:2026-07-14b"
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-dokuwiki; then
    ...
    buildah from --name nodebuilder-dokuwiki -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
